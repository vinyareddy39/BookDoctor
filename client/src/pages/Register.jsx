    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-primary-600 via-primary-700 to-secondary-700 flex-col justify-between p-12 text-white relative overflow-hidden">
        ...
        <div className="z-10 space-y-4">
          ...
          <h2 className="text-4xl font-black leading-tight">Join 5,000+<br />Patients Today.</h2>
          <p className="text-primary-100 text-lg leading-relaxed">
            Create your free account and start booking appointments with the best doctors near you.
          </p>
          <div className="grid grid-cols-2 gap-3 pt-4">
            {[
              { emoji: "🔍", text: "Find specialists" },
              { emoji: "📅", text: "Book instantly" },
              { emoji: "⭐", text: "Rate doctors" },
              { emoji: "🔒", text: "Secure & private" },
            ].map((f) => (
              <div key={f.text} className="flex items-center gap-2 bg-white/10 rounded-xl p-3">
                <span className="text-xl">{f.emoji}</span>
                <span className="text-sm font-semibold text-primary-100">{f.text}</span>
              </div>
            ))}
          </div>
        </div>

        <p className="z-10 text-xs text-primary-300">
          Already have an account?{" "}
          <Link to="/login" className="text-white font-bold underline">Sign in here</Link>
        </p>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-6 py-12 bg-surface">
        <div className="w-full max-w-md">
                ...
                <div className="mt-2 flex gap-1">
                  {[1,2,3,4].map((i) => (
                    <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${
                      password.length >= i * 3
                        ? password.length < 6 ? "bg-red-400" : password.length < 10 ? "bg-amber-400" : "bg-green-500"
                        : "bg-slate-200"
                    }`} />
                  ))}
                </div>
          ...
          <div className="mt-6 space-y-4 text-center text-sm text-slate-500">
            ...
            <Link to="/doctor/register" className="inline-flex items-center gap-1.5 text-accent-600 font-bold hover:underline">
